...
| Language        | TypeScript                        |
| Build tool      | Vite                              |
| Charts          | ECharts (lazy-loaded chunk)       |
| Package manager | pnpm                              |
| Dev containers  | Podman + podman-compose           |
| Production HTTP | Caddy (TLS via Let's Encrypt)     |
| SBOM            | Syft (SPDX-JSON)                  |
| Linting         | ESLint                            |
| Formatting      | Prettier                          |
...
- `expiresAt` — access token expiry, used by the HTTP client for proactive refresh (persisted)
- `currentUser` — logged-in user info (persisted)
...
`accentToCssVars()` looks up the `--accent` / `--accent-dark` / `--accent-light` triple for the chosen accent from a static Material Design palette and applies them to the document root.
...
- `conventional-pre-commit` — enforces [Conventional Commits](https://www.conventionalcommits.org/) on the commit message (commit-msg stage)
- `gitleaks` — secrets detection
- `ensure node_modules` — installs dependencies if missing so the local hooks below can find their plugins
- `eslint` — lints `.ts` and `.svelte` files (auto-fixes where possible)
- `prettier` — formats `.ts`, `.svelte`, `.json`, `.md`, and `.yaml`
- `svelte-check` — type-checks Svelte components
